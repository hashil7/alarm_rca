...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler("rcav3.log", mode="a"),
...
def main():
    while True:
        try:
            ...
            logging.info(f"Processing alarms at {timestamp}")
            for alarm in alarms:
                id = alarm["ID"]
                prob_cause = alarm["PROB_CAUSE"]
                ip = alarm["OBJ_NAME"]
                ne_time = alarm["NE_TIME"]
                logging.info(f"Processing alarm: ID={id}, PROB_CAUSE={prob_cause}, IP={ip}, TIME={ne_time}")
