...
def setup_logging():
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler("byn_training_data.log", mode="a"),
...
def setup_database():
    ...
    create_table_query = """
    CREATE TABLE IF NOT EXISTS dummy_train_data (
...
def generate_training_data_row(rca_id,attrs):
    """Predict root cause using Bayesian model"""

    ups_value = attrs.get('has_ups_low_battery',False)
    link_value = attrs.get('has_link_down',False)
    block_value = attrs.get('block_failed',False)
    
    has_ups_low_battery = bool(ups_value)
    has_link_down = bool(link_value)
    block_failed = bool(block_value)
    ...
    if rca_id:
        if ups_value == rca_id:
            label = "has_ups_low_battery"
        elif link_value == rca_id:
            label = "has_link_down"
        elif block_value == rca_id:
            label = "block_failed"
    
    logging.info(f"Generated training data: Features={features}, Label={label}")
    
    return features, label    

# Process start events for different alarm types
def process_start_event(current_graph, alarm, removed_links, removed_nodes, ups_alarms, cursor, mydb):
    ...
    if prob_cause != 'link_down':
        find_rca_label(current_graph,ip, alarm_id, rca_id, cursor, mydb)
    if rca_id:
        return
...
def find_rca_label(current_graph, ip, alarm_id, rca_id, cursor, mydb):
    ...
    features, label = generate_training_data_row(rca_id, current_graph.nodes[ip])
    features_json = json.dumps(features)

    insert_query = """
    INSERT INTO dummy_train_data (features, label,timestamp) VALUES (%s, %s,%s)
    """
    cursor.execute(insert_query, (features_json, label, datetime.now().strftime('%Y-%m-%d %H:%M:%S')))
    
    mydb.commit()
    logging.info(f"Inserted training data: Features={features_json}, Label={label}")
...
def main():
    ...
    try:
        ...
        for event in timeline_events:
            ...
            if event_type == 'start':
                process_start_event(current_graph, alarm, removed_links, removed_nodes, ups_alarms, cursor, mydb)
